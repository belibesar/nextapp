...
const UserSchema = z.object({
    ...
    role: z.enum(["supplier", "distributor"], { errorMap: () => ({ message: "Role must be either 'supplier' or 'distributor'" }) }),
    companyName: z.string().min(3, { message: "Company name must be at least 3 characters long" }),
    contact: z.object({
        phone: z.string().min(10, { message: "Phone number must be at least 10 characters long" }),
        address: z.object({
            province: z.string().min(3, { message: "Province must be at least 3 characters long" }),
            regency: z.string().min(3, { message: "Regency must be at least 3 characters long" })
        })
    })
});
