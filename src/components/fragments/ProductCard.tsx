    ...
    <div className="bg-white rounded-lg shadow overflow-hidden hover:shadow-lg transition-transform duration-200 ease-in-out hover:scale-105 transform">
      ...
      <div className="p-4">
        <div className="flex flex-row justify-between">
          <h3 className="font-bold">{product.name.length > 15 ? `${product.name.slice(0, 15)}...` : product.name}</h3>
          <h3 className="font-bold">{bundlePrice}</h3>
        </div>

        <div className="flex flex-row justify-between">
          <h3 className="font-bold">{product.producer.name.length > 15 ? `${product.producer.name.slice(0, 15)}...` : product.producer.name}</h3>
        ...
        <div className="flex flex-row justify-between items-start mt-5">
          ...
          <div className="flex flex-col">
            ...
            {product.stock !== 0 ? <p className="text-sm text-blue-500">{product.stock} paket available</p> : <p className="text-sm text-red-500">Out of stock</p>}
          </div>
        </div>
