...
export default function AdminComponentOrderPage({order}: {order: OrderType}) {
    ...
    const getNextStatus = (currentStatus: string) => {
        ...
        const normalizedStatus = currentStatus === 'pending' ? 'PENDING' : currentStatus.toUpperCase();
        const nextStatus = (function() {
            switch (normalizedStatus) {
                case 'PENDING': return 'AWAITING_ADMIN_CONFIRMATION';
                case 'AWAITING_ADMIN_CONFIRMATION': return 'DP_CONFIRMED';
                case 'DP_CONFIRMED': return 'AWAITING_ADMIN_CONFIRMATION_FULL';
                case 'AWAITING_ADMIN_CONFIRMATION_FULL': return 'FULLPAYMENT_CONFIRMED';
                case 'FULLPAYMENT_CONFIRMED': return 'AWAITING_FULL_PAYMENT';
                case 'AWAITING_FULL_PAYMENT': return 'PAID_IN_FULL';
                case 'PAID_IN_FULL': return 'PAID_IN_FULL'; 
                default: return 'AWAITING_ADMIN_CONFIRMATION';
            }
        })();
        return nextStatus;
    };
    ...
    const handleNext = async () => {
        const nextStatus = getNextStatus(order.currentStatus ?? 'AWAITING_ADMIN_CONFIRMATION');
    ...
    const formattedPrice = new Intl.NumberFormat('id-ID', {
        style: 'currency',
        currency: 'IDR',
    }).format(order.totalPrice ?? 0).replace('IDR', 'Rp.')
    return (
    <>
    {/* Order Cards */}
            ...
            <div className="card bg-base-100 shadow-md border border-gray-200">
              <div className="card-body">
                <div className="flex justify-between mb-2">
                  <div>
                    <p>
                      <span className="font-semibold">Order ID:</span> <span className="badge badge-sm">{order._id?.toString()}</span>
                    </p>
                    <p>
                      <span className="font-semibold">Status:</span> <span className="badge badge-warning badge-sm">{order.currentStatus}</span>
                    </p>
                    <p>
                  ...
                  <div className='flex flex-col items-center gap-2'>
                    <p className="badge badge-outline">Ordered At {formattedDate}</p>
                    <div className='flex justify-between flex-row w-full'>
                    <button
                        className={`border-1 p-2 w-20 rounded-md duration-200 
                        ${order.currentStatus === 'PAID_IN_FULL' || order.currentStatus === 'CANCELLED' 
                        ...
                        onClick={handleNext} 
                        disabled={order.currentStatus === 'PAID_IN_FULL' || order.currentStatus === 'CANCELLED'}>
                            next
                    </button>
                    <button
                        className={`border-1 p-2 w-20 rounded-md duration-200 
                ...
                <div className="space-y-4 mt-2">
                  ...
                  <div className="flex justify-end border-t pt-2">
                    <p className="font-semibold bg-[#0088c2] text-white px-4 py-2 rounded-lg">Total: {formattedPrice}</p>
                  </div>
                </div>
              </div>
            </div>
